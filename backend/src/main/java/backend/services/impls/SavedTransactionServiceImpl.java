...
import backend.dto.responses.SavedTransactionResponseDto;
...
import backend.repository.SavedTransactionRepository;
...
@Component
@Slf4j
public class SavedTransactionServiceImpl
        implements SavedTransactionService {
    ...
    public ResponseEntity<ApiResponseDto<?>> createSavedTransaction(
        ...
        try {
            ...
            if (userRepository.existsById(
                ...
                SavedTransaction plannedTransaction =
    ...
    public ResponseEntity<ApiResponseDto<?>> addSavedTransaction(
        ...
        try {
            ...
            if (savedTransactionRepository.existsById(
                    savedTransactionId
            )) {

                SavedTransaction plannedTransaction =
                        savedTransactionRepository
                                .findById(savedTransactionId)
                                .orElse(null);

                transactionRepository.save(
                        savedTransactionToTransaction(
                                plannedTransaction
                        )
                );
                ...
                LocalDate upcomingDate =
                        getUpcomingDate(
                                plannedTransaction.getFrequency(),
                                plannedTransaction.getUpcomingDate()
                        );

                plannedTransaction.setUpcomingDate(
                        upcomingDate
                );
            ...
            throw new UserServiceLogicException(
                    "Failed to add transaction. Try again later"
            );
        }
    ...
    public ResponseEntity<ApiResponseDto<?>> editSavedTransaction(
        ...
        try {
            ...
            if (savedTransactionRepository.existsById(
                ...
                plannedTransaction.setAmount(
                        requestDto.getAmount()
                );

                plannedTransaction.setDescription(
                        requestDto.getDescription()
                );

                plannedTransaction.setFrequency(
                        requestDto.getFrequency()
                );

                plannedTransaction.setUpcomingDate(
                        requestDto.getUpcomingDate()
                );

                plannedTransaction.setCategoryId(
                        requestDto.getCategoryId()
                );
            ...
            throw new UserServiceLogicException(
                    "Failed to edit transaction. Try again later"
            );
        }
    ...
    public ResponseEntity<ApiResponseDto<?>> deleteSavedTransaction(
        ...
        try {
            ...
            if (savedTransactionRepository.existsById(
                ...
                savedTransactionRepository.deleteById(
                        plannedTransactionId
                );
        ...
        try {
            ...
            if (userRepository.existsById(userId)) {
                ...
                List<SavedTransaction> transactions =
                        savedTransactionRepository
                                .findByUserIdOrderByUpcomingDateAsc(
                                        userId
                                );

                List<SavedTransactionResponseDto> response =
                        new ArrayList<>();

                for (SavedTransaction t : transactions) {

                    response.add(
                            savedTransactionToDto(t)
                    );
                }
    ...
    getAllTransactionsByUserAndMonth(
            long userId
    ) throws UserServiceLogicException,
            UserNotFoundException {
    ...
    @Override
    public ResponseEntity<ApiResponseDto<?>> getSavedTransactionById(
            long savedTransactionId
    ) throws UserServiceLogicException,
            TransactionNotFoundException {
        ...
        try {
            ...
            throw new UserServiceLogicException(
                    "Failed to fetch transaction. Try again later."
            );
        }
    ...
    private SavedTransaction savedTransactionDtoToEntity(
            SavedTransactionRequestDto requestDto
    ...
    private Transaction savedTransactionToTransaction(
            SavedTransaction savedTransaction
    ) throws CategoryNotFoundException {

        return new Transaction(
                userRepository
                        .findById(savedTransaction.getUserId())
                        .orElse(null),
    ...
    private LocalDate getUpcomingDate(
            ETransactionFrequency frequency,
    ...
    private SavedTransactionResponseDto savedTransactionToDto(
            SavedTransaction savedTransaction
    ) throws CategoryNotFoundException {
    ...
    private String getDueInformation(
            SavedTransaction transaction
    ) {

        if (transaction.getUpcomingDate() == null) {
            return null;
        }

        if (Objects.equals(
                transaction.getUpcomingDate(),
                LocalDate.now()
        )) {
            return "Due on Today";
        }
        ...
        if (transaction.getFrequency()
                == ETransactionFrequency.MONTHLY
                && transaction.getUpcomingDate()
        ...
        if (transaction.getUpcomingDate()
                .isBefore(LocalDate.now())) {
            ...
            long days =
                    period.getYears() * 365L
                            + period.getMonths() * 30L
                            + period.getDays();
